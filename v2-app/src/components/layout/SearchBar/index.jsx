...
export default function SearchBar({ autoFocus = false, onClose }) {
  ...
  useEffect(() => {
    //autofocus on input field when searchIsOpen
    if (autoFocus) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      //this prevents calling searchMovies on every keystroke instantly
      try {
        //fetches results
        ...
        const unique = results.filter((movie) => {
          //removes duplicates by movie.id
  ...
  return (
    <>
      <Combobox
        ...
        name="movie-search"
        onChange={(value) => {
          //when a suggestion is chosen, navigate to its moviedetails page
              ...
              {/* render the search suggestions from the suggestions array  */}
              {
                suggestions
                  .map((movie, index) => {
                    ...
                    if (isDuplicate) {
                      //if that first index is not the current index, then this item is a duplicate
                      console.warn(
      ...
      {/* background dark overlay whenever suggestions appear  */}
      {suggestions.length > 0 && ( 
        <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.3 }}
        className={styles.darkOverlay}></motion.div>
      )}
    </>
  );
}
